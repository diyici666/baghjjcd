...
ARGL_AUTH=${ARGL_AUTH}
ARGL_DOMAIN=${ARGL_DOMAIN}
SSH_DOMAIN=${SSH_DOMAIN}

# 下载并运行 Argl
check_file() {
  [ ! -e argl ] && wget -O argl https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64 && chmod +x argl
}

run() {
  if [[ -n "\${ARGL_AUTH}" && -n "\${ARGL_DOMAIN}" ]]; then
    if [[ "\$ARGL_AUTH" =~ TunnelSecret ]]; then
      echo "\$ARGL_AUTH" | sed 's@{@{"@g;s@[,:]@"\0"@g;s@}@"}@g' > suidao.json
      cat > suidao.yml << EOF
tunnel: \$(sed "s@.*TunnelID:\(.*\)}@\1@g" <<< "\$ARGL_AUTH")
credentials-file: /app/suidao.json
protocol: http2

ingress:
  - hostname: \$ARGL_DOMAIN
    service: http://localhost:8080
EOF
      [ -n "\${SSH_DOMAIN}" ] && cat >> suidao.yml << EOF
  - hostname: \$SSH_DOMAIN
    service: http://localhost:2222
EOF
    [ -n "\${FTP_DOMAIN}" ] && cat >> suidao.yml << EOF
  - hostname: \$FTP_DOMAIN
...
EOF
      nohup ./argl tunnel --edge-ip-version auto --config suidao.yml run 2>/dev/null 2>&1 &
    elif [[ "\$ARGL_AUTH" =~ ^[A-Z0-9a-z=]{120,250}$ ]]; then
      nohup ./cloudflared tunnel --edge-ip-version auto --protocol http2 run --token ${ARGL_AUTH} 2>/dev/null 2>&1 &
    fi
  else
    nohup ./argl tunnel --edge-ip-version auto --protocol http2 --no-autoupdate --url http://localhost:8080 2>/dev/null 2>&1 &
    sleep 5
    local LOCALHOST=\$(ss -nltp | grep '"argl"' | awk '{print \$4}')
    ARGL_DOMAIN=\$(wget -qO- http://\$LOCALHOST/quicktunnel | cut -d\" -f4)
  fi
}
